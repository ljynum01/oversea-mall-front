  ...
  export default {
    methods: {
      ...
      page(currentPage) {
        ...
        if (JSON.parse(sessionStorage.getItem("user")).role == "merchant") {
          this.axios
            .get(
              "http://localhost:8080/orders/findAllByStatus?status=1&sid=" +
          ...
          this.axios
            .get(
              "http://localhost:8080/orders/findAllByStatus?status=1" +
                "&pageNum=" +
                currentPage +
                "&pageSize=5"
            )
    ...
    created() {
      ...
      if (JSON.parse(sessionStorage.getItem("user")).role == "merchant") {
        ...
        this.axios
          .get("http://localhost:8080/orders/findAllByStatus?status=1")
          .then(function (resp) {
            _this.tableData = resp.data.list;
            _this.pageInfo.total = resp.data.total;
            _this.pageInfo.size = resp.data.size;
